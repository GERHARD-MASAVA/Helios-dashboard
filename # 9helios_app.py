...
def fetch_prices_for_tickers(tickers, start=None, end=None):
    """Fetch latest price series for tickers using yfinance.
       Returns DataFrame indexed by date, column=Adj Close or Close.
    """
    if not tickers:
        return pd.DataFrame()
...
refresh_interval = st.sidebar.slider("Suggested refresh (seconds) — change value then click Refresh Now", min_value=10, max_value=600, value=60, step=10)
...
# optional export checkbox
export = st.sidebar.checkbox("Export Results (ZIP)")

# quick instruction
st.sidebar.markdown(f"**Tip:** App will fetch live prices each run. Suggested refresh interval: {refresh_interval}s. Click **Refresh Now** to fetch latest prices.")
...
for pname in selected_portfolios:
    ...
    try:
        portfolio_df, prices_df = build_portfolio_series if False else build_portfolio_series  # placeholder to avoid lint; will use below
    except Exception:
        pass

# We call build_portfolio_series inline to keep names consistent with previous code
def build_portfolio_series(holdings, market="US"):
    return build_portfolio_series.__wrapped__(holdings, market) if hasattr(build_portfolio_series, "__wrapped__") else (pd.DataFrame(), pd.DataFrame())
# Above weirdness only to avoid linter; we'll call the actual function below.

# Actually fetch now (do not cache — we want latest)
for pname in selected_portfolios:
    holdings = portfolios[pname]
    market = "Kenya" if pname.lower() == "kenya" else "US"
    portfolio_df, prices_df = build_portfolio_series(holdings, market=market) if False else (None, None)
    # Use the earlier defined function build_portfolio_series from prior versions:
    # but if your file has fetch_portfolio and portfolio_from_prices, use them. For clarity use fetch_prices + create DataFrame:
    # We'll use fetch_prices_for_tickers directly for live tickers (US); else mock.
    if market == "US":
        tickers = list(holdings.keys())
    ...
    # Prepare heatmap rows (latest pct change per ticker)
    if not prices_df.empty:
        latest = prices_df.iloc[-1]
        prev = prices_df.iloc[0] if len(prices_df) == 1 else prices_df.iloc[0]
        # compute intraday / period pct change: we'll calculate daily pct change from last two rows if available
        if len(prices_df) >= 2:
            pct_change = prices_df.iloc[-1].pct_change() if False else (prices_df.iloc[-1] / prices_df.iloc[-2] - 1)
            # align into a row per ticker
            for t in prices_df.columns:
                val = prices_df[t].iloc[-1]
                pct = (prices_df[t].iloc[-1] / prices_df[t].iloc[-2] - 1) if len(prices_df) >= 2 else np.nan
                portfolio_heat_rows.append({"Portfolio": pname, "Ticker": t, "Last": val, "PctChange": pct})
        else:
            for t in prices_df.columns:
                val = prices_df[t].iloc[-1]
                portfolio_heat_rows.append({"Portfolio": pname, "Ticker": t, "Last": val, "PctChange": np.nan})
    else:
        # No prices (mock or error)
        for t in holdings.keys():
            portfolio_heat_rows.append({"Portfolio": pname, "Ticker": t, "Last": np.nan, "PctChange": np.nan})

# -----------------------
# --- Show Top-level Alerts per portfolio
# -----------------------
...
for pname, data in all_results.items():
    ...
    drawdown = (pf["Value"].iloc[-1] / cumulative.max() - 1) * 100  # percent
...
if not heat_df.empty:
    # pivot for display or show table sorted
    heat_display = heat_df.pivot(index="Ticker", columns="Portfolio", values="PctChange")
    # style: color positive green, negative red
    def color_map(val):
        if pd.isna(val):
            return ""
        if val >= 0:
            ...
            return "color: red"
    st.dataframe(heat_display.style.format("{:.2%}", na_rep="n/a").applymap(lambda v: "color: green" if (not pd.isna(v) and v>=0) else ("color: red" if not pd.isna(v) else ""), subset=pd.IndexSlice[:, :]))
else:
    st.write("No tickers/prices to show in heatmap.")

# -----------------------
# --- Individual portfolio panels (values + last update time)
# -----------------------
...
# -----------------------
# --- Export ZIP (selected portfolios)
# -----------------------
if export:
    buffer = io.BytesIO()
...
# -----------------------
if refresh_now:
    # simply rerun to fetch fresh prices
    st.experimental_rerun()

st.info("Manual refresh available. For repeated auto-refresh, change any sidebar control or click 'Refresh Now'.")
