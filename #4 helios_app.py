...
if export:
    ...
    with zipfile.ZipFile(buffer, "w") as zf:
        zf.writestr(f"Helios_Simulations_{market}.csv", sims_df.to_csv(index=False))
        zf.writestr(f"Helios_OptimalAllocation_{market}.csv", opt_weights_df.to_csv())
        zf.writestr(f"Helios_{market}_Scenario_{scenario.replace(' ','_')}.csv", stressed_df.to_csv())
    buffer.seek(0)
    ...
    st.download_button(
        ...
        data=buffer,
        file_name=f"Helios_Results_{market}.zip",
