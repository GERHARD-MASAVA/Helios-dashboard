...
# -----------------------
# --- Kenya CBK Yield Curve (with caching)
# -----------------------
@st.cache_data(ttl=3600)  # cache for 1 hour
def fetch_cbk_yields():
    """
    Scrape Kenya CBK Treasury Bill rates dynamically.
    """
    url = "https://www.centralbank.go.ke/treasury-bills-bonds/"
    try:
        r = requests.get(url, timeout=10)
        soup = BeautifulSoup(r.text, "html.parser")

        # Example scrape: get all table data
        ...
        st.error(f"Failed to fetch CBK yields: {e}")
        return pd.DataFrame()
...
# --- Kenya Yield Curve
# -----------------------
st.header("🇰🇪 Kenya T-Bill Yield Curve")
cbk_df = fetch_cbk_yields()
if not cbk_df.empty:
    fig, ax = plt.subplots(figsize=(6, 3))
    ax.plot(cbk_df["Tenor"], cbk_df["Rate"], marker="o")
    ax.set_title("Kenya Treasury Bill Yields (Latest)")
    ax.set_ylabel("Rate")
    st.pyplot(fig)
    st.dataframe(cbk_df)
else:
    st.write("No CBK yield data available right now.")
