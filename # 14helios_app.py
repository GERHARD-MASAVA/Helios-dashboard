...
# -----------------------
# --- Kenya Fixed Income Dashboard
# -----------------------
def get_cbk_tbills():
    """Mock Kenya CBK T-bill rates (in %)."""
    data = {
        "Tenor (Days)": [91, 182, 364],
        "Yield (%)": [15.1, 15.4, 15.7]  # Replace with live scrape later
    }
    return pd.DataFrame(data)

def get_kenya_eurobonds():
    """Mock Kenya Eurobond yields (in %)."""
    data = {
        "Maturity (Years)": [2, 5, 10],
        "Yield (%)": [9.2, 10.5, 11.8]  # Replace with live API later
    }
    return pd.DataFrame(data)
...
tbs = get_cbk_tbills()
euro = get_kenya_eurobonds()

col1, col2 = st.columns(2)

with col1:
    st.subheader("CBK T-Bill Yields")
    st.table(tbs)

    fig, ax = plt.subplots()
    ax.plot(tbs["Tenor (Days)"], tbs["Yield (%)"], marker="o", label="T-bills")
    ax.set_title("Kenya T-Bill Yield Curve")
    ax.set_xlabel("Tenor (days)")
    ax.set_ylabel("Yield (%)")
    st.pyplot(fig)

with col2:
    st.subheader("Kenya Eurobond Yields")
    st.table(euro)

    fig, ax = plt.subplots()
    ax.plot(euro["Maturity (Years)"], euro["Yield (%)"], marker="s", color="orange", label="Eurobonds")
    ax.set_title("Kenya Eurobond Yield Curve")
...
spread = euro["Yield (%)"].mean() - tbs["Yield (%)"].mean()
st.write(f"📊 Average Eurobond – T-Bill spread = {spread:.2f}%")
...
if export:
    ...
    with zipfile.ZipFile(buffer, "w") as zf:
        ...
        # Add fixed income data too
        zf.writestr("Kenya_TBills.csv", tbs.to_csv(index=False))
        zf.writestr("Kenya_Eurobonds.csv", euro.to_csv(index=False))
    buffer.seek(0)
